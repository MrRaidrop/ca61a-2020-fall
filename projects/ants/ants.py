...
class Ant(Insect):
    ...
    def add_to(self, place):
        if place.ant is None:
            ...
            ant_placed=place.ant
            if ant_placed.can_contain(self):
                ant_placed.contain_ant(self)
            elif self.can_contain(ant_placed):
                place.ant=self
                self.contain_ant(ant_placed)
            else:
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, beehive):
        ...
        target=self.place
        location=0
        while target.entrance!=beehive:
            if target.bees!=[] and self.min_range<=location<=self.max_range:
                break
            else:
        ...
        if target==beehive:
            return None
        return rANTdom_else_none(target.bees) # REPLACE THIS LINE
        # END Problem 3 and 4
...
class FireAnt(Ant):
    ...
    def reduce_armor(self, amount):
        ...
        if amount>=self.armor:
            current_damage=self.damage+amount
        else:
...
# END Problem EC
    ...
    def __init__(self, armor=1):
        ...
        if self.is_the_true_queen==1:
            QueenAnt.is_the_true_queen=0
            self.is_the_true_queen=1
        # END Problem EC
    ...
    def reduce_armor(self, amount):
        ...
        if self.is_the_true_queen and self.armor == 0:
            bees_win()
        # END Problem EC
...
class ScaryThrower(ThrowerAnt):
    ...
    def throw_at(self, target):
        # BEGIN Problem Optional 4
        "*** YOUR CODE HERE ***"
        if target and not (target in self.scared_bees):
