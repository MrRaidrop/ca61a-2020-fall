...
def scheme_eval(expr, env, _=None): # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first in SPECIAL_FORMS:
        ...
        body=rest.map(lambda x: scheme_eval(x, env))
        return scheme_apply(head,body,env)
        # END PROBLEM 4
...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        lambda_exp=Pair(formals,body)
        env.define(name,do_lambda_form(lambda_exp,env))
        return name
        # END PROBLEM 9
    else:
...
def do_cond_form(expressions, env):
    ...
    while expressions is not nil:
        ...
        if is_true_primitive(test):
            ...
            if clause.rest==nil:
                return test
            while clause.rest!=nil:
                result=scheme_eval(clause.rest.first,env)
                clause=clause.rest
            return result
            # END PROBLEM 13
        expressions = expressions.rest
