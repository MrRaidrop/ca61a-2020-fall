class VendingMachine:
    ...
    def vend(self):
        ...
        elif self.fund<self.cost:
            return ('You must add ${} more funds.'.format(self.cost-self.fund))
        else:
            if self.fund==self.cost:
                self.stock=self.stock-1
                self.fund=0
                return ('Here is your {}.'.format(self.name))
            else:
                self.stock=self.stock-1
                self.carge=self.fund-self.cost
                self.fund=0
                return ('Here is your {} and ${} change.'.format(self.name,self.carge))
...
def store_digits(n):
    ...
    #a tricky way
    def helper(n):
        count,result,tem=0,[],n
        while n>10:
            count+=1
            n=n//10
        for i in list(range(count))[::-1]:
            result.append(tem//pow(10,i+1))
        result.append(tem%10)
        return result
    result=Link.empty
    for i in helper(n)[::-1]:
        result=Link(i%10,result)
    return result
