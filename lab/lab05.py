...
def distance(city_a, city_b):
    ...
    "*** YOUR CODE HERE ***"
    a,b=city_a,city_b
    return sqrt((get_lon(a)-get_lon(b))**2+(get_lat(a)-get_lat(b))**2)
...
def add_trees(t1, t2):
    ...
    zz=list(zip(t1, t2))
    if len(zz) > 1:
        result_branches = [add_trees(zz[i][0],zz[i][1]) for i in range(1,len(zz))]
    else:
        result_branches = []
    added_tree = tree(sum(zz[0]), result_branches)
    return added_tree
