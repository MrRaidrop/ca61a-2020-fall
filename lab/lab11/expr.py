...
class CallExpr(Expr):
    ...
    def eval(self, env):
        ...
        operands=[opd.eval(env) for opd in self.operands]
...
class LambdaFunction(Value):
    ...
    def apply(self, arguments):
        ...
        "*** YOUR CODE HERE ***"#can't figue out why in the new env the parameter need to be bond to the args
        env_copy=self.parent.copy()
        hint=zip(self.parameters,arguments)
        for parameter, argument in hint:
            env_copy[parameter]=argument
