...
class Keyboard:
    ...
    def press(self, info):
        ...
        if 0<=info<len(self.buttons):
            pressed=self.buttons[info]
            pressed.times_pressed+=1
            return pressed.key
        return ''
    ...
    def typing(self, typing_input):
        ...
        for i in typing_input:
            result=result+self.buttons[i].key
...
def trade(first, second):
    ...
    equal_prefix = lambda: sum(i for i in first[:m])==sum(j for j in second[:n]) 
    while m<=len(first) and n<=len(second) and equal_prefix()==False:
        if sum(i for i in first[:m])<sum(j for j in second[:n]):
            m += 1
        else:
            n += 1
